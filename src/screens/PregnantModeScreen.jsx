...
export default function PregnantModeScreen() {
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={{ ...styles.header, background: GRADIENTS.purpleHeader }}>
        <button onClick={() => navigate("/")} style={styles.back}>← Back</button>
        <div style={styles.headerEmoji}>🤰</div>
        <h1 style={styles.headerTitle}>Get Pregnant</h1>
        <p style={styles.headerSub}>Track fertility &amp; ovulation windows</p>
        <div style={styles.headerGlow} />
      </div>

      <div style={styles.body}>
        <div className="fade-up-1">
          <Card accentColor="#6C63FF">
            <p style={{ ...styles.cardLabel, color: "#a89cff" }}>📅 Fertility Window</p>
            <p style={styles.cardBody}>
              Understand your most fertile days and boost your chances naturally with smart tracking.
            </p>
            <div style={{ ...styles.badge, background: "rgba(108,99,255,0.2)", color: "#a89cff", border: "1px solid rgba(108,99,255,0.3)" }}>
              ● Ovulation insights ready
            </div>
          </Card>
        </div>
...
const styles = {
  ...
  blob1: { position: "fixed", top: "-100px", right: "-100px", width: "380px", height: "380px", borderRadius: "50%", background: "radial-gradient(circle, rgba(108,99,255,0.28) 0%, transparent 70%)", animation: "blobPulse 9s ease-in-out infinite", pointerEvents: "none", zIndex: 0, filter: "blur(2px)" },
  blob2: { position: "fixed", bottom: "-80px", left: "-80px", width: "300px", height: "300px", borderRadius: "50%", background: "radial-gradient(circle, rgba(59,55,204,0.2) 0%, transparent 70%)", animation: "blobPulse 12s 2s ease-in-out infinite", pointerEvents: "none", zIndex: 0 },
